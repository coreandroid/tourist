...
$Data['Contact'] ="";
 ...
 $Data['Image'] ="";
   ...
   $result = mysqli_query($conn,"select * from resorts where Id=$_GET[Id]");
  ...
  move_uploaded_file($_FILES["image"]["tmp_name"],'images/'.$_FILES["image"]["name"]);
     ...
     if(intval($_POST['Id']) > 0){ // if hash id then update
      ...
      $stmt = $conn->prepare("UPDATE resorts set `Name`=?,`OpenTime`=?,`CloseTime`=?,`MinimumSpend`=?,`Location`=?,`Contact`=?,`Description`=?,`Website`=?,`Image`=? where Id=?");
 ...
 echo "<script>alert('Resort is Updated');window.location.assign('resort.php');</script>";
     }else { //if not then add

     $stmt = $conn->prepare("INSERT INTO resorts (`Name`, `OpenTime`, `CloseTime`, `MinimumSpend`, `Location`, `Contact`, `Description`, `Website`,`Image`) VALUES (?, ?, ?,?,?,?,?,?,?)");
   ...
   echo "<script>alert('Resort is Added');window.location.assign('resort.php');</script>";
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header" data-background-color="purple">
                                    <h4 class="title">Resort Edit</h4>
                                    <p class="category">Edit or Create Resort</p>
                                </div>
                                           ...
                                           <div class="row">
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group label-floating">
                                                    ...
                                                    <input type="text" value="<?php echo $Data['Location'];?>"  name="location" class="form-control">
                                               
                                                </div>
                                          </div>

                                     <?php if(strlen($Data['Name']) > 0){ ?>
                                          <iframe  width="100%" height="400" frameborder="0" style="border:0" allowfullscreen src="//www.google.com/maps/embed/v1/place?q=<?= $Data['Name'];?> <?= $Data['Location'];?>&zoom=17&key=AIzaSyCDmf5-MFqs4IO4CYfMIF3z2TXhEoO7HYg">
                                         </iframe>
                                         <?php } ?>
                                        ...
                                        <input type="submit" class="btn btn-primary pull-right" name="save" value="Save Resort">
